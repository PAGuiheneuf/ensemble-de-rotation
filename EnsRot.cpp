  ...
  for(int k = 0 ; k < M+1 ; k++){
    ...
    for(int n = 1 ; n < Tps+1 ; n++){
      if(omp_get_thread_num()==0){
        std::cout << "Thread 0 : k = " << k << ", n = " << n << std::endl;
      }
      double maxE = this->path_bary(n, t, u);
...
double Graph::path_bary(int n, double t, double u){
  double maxE = 0;
  ...
  for(int i = 0 ; i < this->edges2.size() ; i++){
    ...
    if (e->et2 >= n){
      ...
      for(int j = 0 ; j < e->end->edges1.size() ; j++){
        ...
        Edge2* e2 = f->edge2;
        if(e2->et2 == n+1){
          e2->et1 = std::max( e2->et1 , eti + t*f->et0 + u*f->et1);
          maxE = std::max(maxE, e2->et1);
        }
        else if(e2->et2 == n){
          e2->et0 = e2->et1;
          e2->et1 = eti + t*f->et0 + u*f->et1;
          e2->et2 = n+1;
          maxE = std::max(maxE, e2->et1);
        }
      }
...
//Zone parallèle
#pragma omp parallel for schedule(dynamic) shared(ROTS)
