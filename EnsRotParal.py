...
def compute_the_rotation_ensemble(graph):
    ...
    for k in range(M+1):
        ...
        for n in range (1,Tps+1):
            ...
            rotTemp.append(maxE)
        fit = numpy.polyfit(range(len(rotTemp)-30,len(rotTemp)+1), rotTemp[len(rotTemp)-31:len(rotTemp)], 1)
...
def do_the_whole_work(p):
    ...
    print "Graphe a " + str(len(g.nodes)) + " noeuds, " + str(len(g.edges)) + " edges1, " + str(len(g.edges2)) + " edges2."
    print 'Construction du graphe en : ' + repr(round(100*(time.time()-debut))/float(100)) + ' s'
    ...
    print "Calcul de l'ensemble de rotation en :" + repr(round(100*(time.time()-debut2))/float(100)) + ' s'
    print "Ensemble de rotation = " + str(rot)
...
N   = 20   #Nb de gros carrés
d   = 16   #Nb de petits carrés dedans
Tps = 50  #Nb d'itérations
M   = 8   #Nb d'angles utilisés pour la représentation

"""
N   = 10;  #Nb de gros carrés
d   = 16;  #Nb de petits carrés dedans
Tps = 20;  #Nb d'itérations
M   = 8;   #Nb d'angles utilisés pour la représentation
"""

if __name__ == "__main__":
    #pool      = multiprocessing.Pool(4) # 15 = nb CPUS à utiliser (mon ordinateur a 16 coeurs)
    t0 = time.time()
    #durations = pool.map(do_the_whole_work, range(1)) # range(100) = liste distribuée à la fonction do_the_whole_work
    for i in range(10):
        do_the_whole_work(i)
    #print durations
    print "Temps total de calcul = ", time.time() - t0, "s"
