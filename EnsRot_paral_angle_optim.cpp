...
#include "EnsRot_paral_angle_optim.h"
...
Edge2::Edge2(Node& _start, Node& _end, double _et0, double _et1, double _et2){
  this->etiks = std::vector<double>(3*(M+1), 0.);
  for(int i = 0 ; i < M+1 ; i++){
    this->etiks[3*i + 0] = _et0;
    this->etiks[3*i + 1] = _et1;
    this->etiks[3*i + 2] = _et2;
  }
  this->start = &_start;
  this->end   = &_end;
...
std::vector<double> Graph::compute_the_rotation_ensemble(){
  ...
  for (int l = 0 ; l < this->edges2.size() ; l++){
    ...
    this->edges2[l]->etiks = std::vector<double>(3*(M+1), 0);
    for(int i = 0 ; i < M+1 ; i++){
      this->edges2[l]->etiks[3*i+2] = 1;
    }
  }
  ...
  for(int i = 0 ; i < this->edges2.size() ; i++){
    ...
    if (e->etiks[3*m+2] >= n){
      ...
      for(int j = 0 ; j < e->end->edges1.size() ; j++){
        ...
        Edge2* e2 = f->edge2;
        if(e2->etiks[3*m+2] == n+1){
          e2->etiks[3*m+1] = std::max( e2->etiks[3*m+1] , eti + t*f->et0 + u*f->et1);
          maxE = std::max(maxE, e2->etiks[3*m+1]);
        }
        else if(e2->etiks[3*m+2] == n){
          e2->etiks[3*m+0] = e2->etiks[3*m+1];
          e2->etiks[3*m+1] = eti + t*f->et0 + u*f->et1;
          e2->etiks[3*m+2] = n+1;
          maxE = std::max(maxE, e2->etiks[3*m+1]);
        }
      }
